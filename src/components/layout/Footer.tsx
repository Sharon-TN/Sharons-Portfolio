...
import { Heart, Coffee, Code, Sparkles, Mail } from "lucide-react";
      ...
      <div className="section-container py-12 sm:py-16">
        <div className="grid sm:grid-cols-3 gap-10">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--color-text-muted)] max-w-xs leading-relaxed">
              {personalInfo.professionalTitle} — {personalInfo.year} {personalInfo.degree} student at {personalInfo.university}.
            </p>
          </div>

          {/* Navigation */}
          <div>
            <h4 className="text-xs font-mono uppercase tracking-wider text-[var(--color-text-muted)] mb-4">
              Navigate
            </h4>
            <div className="grid grid-cols-2 gap-2">
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className="text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-accent-primary)] transition-colors duration-200"
                >
                  {link.label}
                </a>
              ))}
        ...
        >
          ...
          <motion.p
            ...
            viewport={{ once: true }}
            className="text-sm text-[var(--color-text-muted)] flex items-center gap-1.5 flex-wrap justify-center"
          >
